...
def validate_metadata(metadata_file):
    ...
    cache = {
        "https://raw.githubusercontent.com/bio-ontology-research-group/hguploader/master/hguploader/schema.yml": schema_resource.read().decode(
            "utf-8")}
    (document_loader,
     avsc_names,
     schema_metadata,
...
@ck.option('--bed-file-grch37', '-bf37',
           help='Patient\'s Orignial BED file with assembly version GRCh37(hg19) for exome uploads')
...
@ck.option('--father-bed-file-grch37', '-fbf37',
           help='Father\'s orignial BED file with assembly version GRCh37(hg19) for exome uploads')
...
@ck.option('--mother-bed-file-grch37', '-mbf37',
           help='Mother\'s Orignial BED file with assembly version GRCh37(hg19) for exome uploads')
...
@ck.option('--sibling-bed-file-grch37', '-sbf37',
           help='Sibling\'s Orignial BED file with assembly version GRCh37(hg19) for exome uploads')
...
def main(uploader_project, sequence_read1, sequence_read2, bed_file, bed_file_grch37,
    ...
    sample_id = str(metadata['id']) + '_' + metadata['patient']['identifier']
    if upload_id:
    ...
    if not no_sync:
        ...
        requests.post(UPLOADER_URL + '/api/sync', verify=False, data=data)
